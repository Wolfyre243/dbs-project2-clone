        ...
        if (subtitle && subtitle.wordTimings?.length) {
          ...
          const currentWordIndex = subtitle.wordTimings.findIndex(
            (timing) =>
              ...
              currentTime < timing.end + 0.1, // Buffer to avoid flicker
          );
          ...
          .map((subtitle) => {
            ...
            const units = subtitle.wordTimings?.length
              ? subtitle.languageCode === 'cmn-CN'
                ? subtitle.wordTimings.reduce((acc, timing, index) => {
                    if (index % 4 === 0) {
                      acc.push({ word: '', start: timing.start, end: timing.end });
                    }
                    acc[acc.length - 1].word += timing.word;
                    if (index % 4 !== 3 && index < subtitle.wordTimings.length - 1) {
                      acc[acc.length - 1].end = subtitle.wordTimings[index + 1].end;
                    }
                    return acc;
