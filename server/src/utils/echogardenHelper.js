...
module.exports.generateWordTimings = async (audioUrl, text, languageCode) => {
  try {
    console.log('Calling Google Speech-to-Text with:', { audioUrl, text, languageCode });

    // Download audio from Supabase
    const audioResponse = await axios.get(audioUrl, { responseType: 'arraybuffer' });
    const audioContent = Buffer.from(audioResponse.data).toString('base64');

    const mappedLanguage = languageCode === 'cmn-CN' ? 'cmn-Hans-CN' : languageCode || 'en-US';
    ...
    const wordTimings = results
      ...
      .map((wordInfo) => ({
        word: wordInfo.word,
        start: parseFloat(wordInfo.startTime.seconds || 0) + (wordInfo.startTime.nanos || 0) / 1e9,
        end: parseFloat(wordInfo.endTime.seconds || 0) + (wordInfo.endTime.nanos || 0) / 1e9,
      }));
