...
app.get('/profile', async (req, res) => {
  ...
  try {
    const result = await db.query(`
      ...
      WHERE users.id = $1
    `, [1]); // Replace with actual user ID from token in production
...
app.get('/users', async (req, res) => {
  ...
  try {
    ...
    const usersResult = await db.query(usersQuery, [`%${search}%`, limit, offset]);
...
app.post('/auth/login', async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ accessToken: 'mockAccessToken', refreshToken: 'mockRefreshToken', role: user.role });
  } catch (error) {
